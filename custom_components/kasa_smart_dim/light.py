...
async def async_setup_entry(
    ...
    async_add_entities([KasaSmartDimmer(coordinator, entry.title)], True)
...
class KasaSmartDimmer(CoordinatorEntity, LightEntity):
    ...
    async def async_turn_on(self, **kwargs: Any) -> None:
        ...
        _LOGGER.debug(
            f"Coordinator device: {self.coordinator.device}, host: {self.coordinator.device.host}"
        )
        ...
        try:
            ...
            if ATTR_TRANSITION in kwargs and physical_is_on:
                ...
                _LOGGER.debug(
                    f"Setting brightness to {brightness_pct}%% with transition {transition_ms}ms for {self.name}"
                )
                await self.coordinator.device.set_dimmer_transition(
                    brightness=brightness_pct, transition=transition_ms
                )
            else:
                # If a transition was requested but the light is off, log it and set brightness without transition.
                if ATTR_TRANSITION in kwargs:
                    _LOGGER.debug(
                        f"Ignoring transition for {self.name} because physical light is off."
                    )
                _LOGGER.debug(
                    f"Setting brightness to {brightness_pct}%% for {self.name}"
                )
            ...
            _LOGGER.error(f"Failed to set brightness for {self.name}: {e}")
            # Refresh the state to ensure we have the correct data after a failed update.
            await self.coordinator.async_refresh()
